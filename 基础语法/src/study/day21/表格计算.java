...
public class 表格计算 {
    ...
    private static double std(int[] index) {
        double avg=avg(index);
        double count=(index[2]-index[0]+1)*(index[3]-index[1]+1);
        double ans = 0;
        for (int i = index[0]-1; i <index[2] ; i++) {
            for (int j = index[1]-1; j <index[3] ; j++) {
                if (Character.isDigit(data[i][j].charAt(0))){
                    ans+=(Double.parseDouble(data[i][j])-avg)*(Double.parseDouble(data[i][j])-avg);
                }else {
                    ans+=(f(data[i][j])-avg)*(f(data[i][j])-avg);
                }
            }
        }
        return Math.sqrt(ans/count);
    }

    private static double avg(int[] index) {
        double sum=sum(index);
        double count=(index[2]-index[0]+1)*(index[3]-index[1]+1);
        return sum/count;
    }
